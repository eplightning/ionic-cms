...
abstract class Controller {
	...
	public static function call($destination, $parameters = array())
	{
		...
		$controller = static::resolve(DEFAULT_BUNDLE, $name);
	...
	public static function resolve($bundle, $controller)
	{
		...
		if (IoC::registered('controller: '.$controller))
		{
			return IoC::resolve('controller: '.$controller);
		}
	...
	protected static function load($bundle, $controller)
	{
		...
		if (is_file($path = path('app').'controllers/'.$controller.EXT))
		{
			require_once $path;
        ...
        if ($path = Event::until('ionic.controller_path', array($controller)))
        {
            require_once $path;
