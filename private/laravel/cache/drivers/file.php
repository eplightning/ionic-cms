...
class File extends Driver {
    ...
    {
        if (is_file($this->path.$key)) @unlink($this->path.$key);
    }
    ...
    public function list_all()
    {
        ...
        foreach (new FilesystemIterator($this->path, FilesystemIterator::SKIP_DOTS) as $f) {
            // Ignore directories and other non-file things
            if ($f->isFile()) {
                $list[] = array(
                    'cache_name' => $f->getFilename(),
                    'expiration' => (int) substr(file_get_contents($f->getPathname()), 0, 10)
                );
            }
        }
    ...
    {
        $results = glob($this->path.$key, GLOB_NOSORT);
